...
def process_raw_binning(buffer):
    rawdata = np.asarray(buffer, 'uint32').reshape(2084*2,3112*2)
...
def process_low_res(buffer):
    rawdata = np.asarray(buffer, 'uint16').reshape(2084,3104)
    red_channel = rawdata[1::2, 1::2]

    g_0 = rawdata[0::2, 1::2]
    #g_1 = rawdata[1::2, 0::2]
    #green_channel = np.mean(np.array([g_0, g_1]), axis=0).astype('uint8')

    blue_channel = rawdata[0::2, 0::2]

    return (np.asarray([red_channel, g_0, blue_channel]).transpose(1,2,0))
